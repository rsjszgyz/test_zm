...
Customer.prototype.listen("order", waiter1);
...
function loopCustomer(customer, seat) {
    ...
    //发布点菜消息
    let status = customer.trigger("order", customer);

    //若没有响应
    if (!status) {
        let t = setInterval(function () {
            status = customer.trigger("order", customer);
            if (status) {
                clearInterval(t);
            }
        }, 2300);
    ...
    if (target.nodeName.toLowerCase() == "button") {
        ...
        switch (id) {
            ...
            case "button3":
                ...
                Waiter.prototype.remove("cook", cooks[index]);
                break;
